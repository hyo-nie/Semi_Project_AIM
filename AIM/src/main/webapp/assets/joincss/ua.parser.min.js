 ...
 * Licensed MIT - https://github.com/daumcorp/ua_parser/blob/master/LICENSE*/
 ...
 function viewport(){
  var userAgent = daumtools.userAgent();
  var $ua = userAgent.ua;
  var $os = userAgent.os.name;
  var $version = userAgent.os.version.major;
  var $device = userAgent.platform;
  var $browser = userAgent.browser.name;
  if( $(window).width() >= 1025 ){
    if( $('html').hasClass('laptop') ) return false;
    $('html').removeAttr('class').addClass('laptop '+$browser+' '+$os+' '+$version);
  }else if( $(window).width() >=680 ){
    if( $('html').hasClass('tablet') ) return false;
    $('html').removeAttr('class').addClass('tablet '+$browser+' '+$os+' '+$version);
  }else{
    if( $('html').hasClass('mobile') ) return false;
    $('html').removeAttr('class').addClass('mobile '+$browser+' '+$os+' '+$version);
  }
}
